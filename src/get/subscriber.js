...
export default async (client, command) => {
  ...
  if (subscriber.exists) {
   ...
   await client.utility.subscriber.avatar(subscriber.id, IconSize.SMALL)
      ...
      .catch(async () => await client.messaging.sendGroupMessage(
        command.targetGroupId,
        client.phrase.getByLanguageAndName(command.language, `${client.config.keyword}_subscriber_no_avatar_message`)
    ));
  */
